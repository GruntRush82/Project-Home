...
def _smtp_params() -> dict[str, Any] | None:
...
def _send_email(msg: EmailMessage) -> None:
    ...
    with smtplib.SMTP(params["host"], params["port"]) as s:
        s.starttls()
        s.login(params["user"], params["pwd"])
        s.send_message(msg, from_addr=params["from_addr"], to_addrs=msg["To"])
